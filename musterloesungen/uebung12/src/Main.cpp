...
void setupGame(string filename, MainWindow* w, Game* game)
{
	 ...
	 BOOST_FOREACH(const ptree::value_type&  v, pt.get_child("level") )
	 {
		 ...
		 if( v.first == "pathFollower")
		 {
			 ...
			 // Get texture
			 SDL_Texture* texture = TextureFactory::instance(w->getRenderer()).getTexture(path + "/" + filename);
			 PathFollower* p = new PathFollower(w->getRenderer(), texture, frameWidth, frameHeight, numFrames);
			 p->setFPS(fps);
			 game->setPathFollower(p);
		 }
		 if( v.first == "network")
		 {
			 string filename = v.second.get("<xmlattr>.filename", "");
			 unsigned int start = v.second.get<unsigned int>("start", 0);
			 unsigned int goal = v.second.get<unsigned int>("goal", 0);
			 Network* net = new Network(w->getRenderer(), path + "/" + filename);
			 net->setStart(start);
			 net->setGoal(goal);
			 game->setNetwork(net);
